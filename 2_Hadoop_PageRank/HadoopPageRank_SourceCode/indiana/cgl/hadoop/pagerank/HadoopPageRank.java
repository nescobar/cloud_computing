...
import indiana.cgl.hadoop.pagerank.helper.SortRankValueMap;
import indiana.cgl.hadoop.pagerank.helper.SortRankValueReduce;
...
public class HadoopPageRank extends Configured implements Tool{
	...
	public static void main(String[] args) throws Exception {
		...
		FileOutputFormat.setOutputPath(job3, new Path(String.valueOf(outputIndex+1)));
		...
		System.out.println("Hadoop SortRankValue starts...\n");
		/////////////////////////////////////////////////////
		//collect the (url_id,rank_value) tuple from output in stage 2 
		
		Job job4 = new Job(config, "SortRankValue");

		job4.setJarByClass(HadoopPageRank.class);
		job4.setMapperClass(SortRankValueMap.class);
		job4.setReducerClass(SortRankValueReduce.class);
		...
		FileInputFormat.setInputPaths(job4, new Path(String.valueOf(outputIndex)));
		FileOutputFormat.setOutputPath(job4, new Path(String.valueOf(outputDir)));
		...
		if (!job4.isSuccessful()){
			System.out.println("Hadoop SortRankValue failed, exit...");
			System.exit(-1);
		}
