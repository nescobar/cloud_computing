...
/**
 * Created by Nicolas Escobar on 4/13/17.
 */
public class KmeansMiniBatchMapCollective extends Configured implements Tool {
    ...
    void launch(int batchSize, int numOfTasks, int numOfIterations, int numOfCentroids, String workDir)
        ...
        Path centroidsDir = new Path(workDirPath, "centroids");
        ...
        //
        runMbKMeans(batchSize, numOfIterations, numOfCentroids, jobId, numOfTasks, configuration, workDirPath, dataDir, centroidsDir, outDir);
    ...
    private void runMbKMeans(int batchSize, int numOfIterations, int numOfCentroids,
                           ...
                           Path workDirPath, Path dataDir, Path cenDir, Path outDir)
        ...
        do {
            ...
            Job kmeansJob = configureMBKMeansJob(batchSize, numOfMapTasks, numOfCentroids,
                    configuration, workDirPath, dataDir,cenDir, outDir, JobID, numOfIterations);
    ...
    private Job configureMBKMeansJob(int batchSize,
                                   int numMapTasks, int numOfCentroids, Configuration configuration,Path workDirPath, Path dataDir, Path cDir,
        ...
        Path cFile = new Path(cDir,KMeansMiniBatchConstants.CENTROID_FILE_PREFIX + jobID);
        System.out.println("Centroid File Path: "+ cFile.toString());

        // Set Job Configuration Constants
        jobConfig.set(KMeansMiniBatchConstants.CFILE,cFile.toString());
