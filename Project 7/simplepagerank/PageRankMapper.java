...
public class PageRankMapper extends CollectiveMapper<String, String, LongWritable, Text> {
	...
	public void mapCollective( KeyValReader reader, Context context) throws IOException, InterruptedException {
		...
		Long2DoubleKVTable initPRTable = new Long2DoubleKVTable(1, new TypeDoubleCombiner());
		initPRTable(partialGraph, initPRTable);
		allgather("harp-pagerank",  "allgather_initPRTable",initPRTable);
		...
		for(int i=0; i<noIterations; i++){
			...
			globalPRTable = new Long2DoubleKVTable(2,new TypeDoubleCombiner());
			...
			allreduce("harp-pagerank",  "allgather_"+i,globalPRTable);
	...
	public void updatePRTable( Long2DoubleKVTable globalPRTable){
		for(Partition<Long2DoubleKVPartition> aPartition: globalPRTable.getPartitions()){
			int key = aPartition.id();
			double sumOfRankValues = aPartition.get().getVal(key);
	...
	public void computePartialPR(Map<Long, ArrayList<Long>> partialGraph, Long2DoubleKVTable localPRTable, Long2DoubleKVTable globalPRTable){
		...
		for (Entry<Long, ArrayList<Long>> entry : partialGraph.entrySet()) {
		    ...
		    if(targetUrls == null ){
				...
				// Iterate over list of IDs and add PR to each node in the global PR table
				// If globalPRTable.get(i) exists, then it will add pr to it
				// If globalPRTable.get(i) does not exist, then it will create a new entry with the PR
				...
				// If targetURL exists, then pr will be added to existing

				for (long target : targetUrls){
					System.out.println("Key Target: " + target + " Value:" + pr);
					globalPRTable.addKeyVal(target, pr);
				}

				// This is another way of achieving the same functionality
				/*
				for (int i=0; i<numOfOutLinks; i++){
					// Iterate over list of target URLs and add <targetURL, PR> to the global table
					globalPRTable.addKeyVal(targetUrls.get(i), pr);
					System.out.println("Key Target: " + targetUrls.get(i) + " Value:" + pr);
				}
				*/
